...
namespace Pacman.Simulator
{
	public class Map
	{
		...
		public void Reset(){
            ...
            PowerPillsLeft = PowerPillNodes.Count;
        ...
        public static int GetNumberOfJunctionsInPath(Node startNode, Node endNode, Direction initialDir) {
            ...
            while (pathInfo.ParentNode != startNode) {
                if (pathInfo.ParentNode.PossibleDirections.Count > 2) nJunctions++;
		...
		private void analyzeShortestPath() {
            foreach (Direction initialDir in Enum.GetValues(typeof(Direction)))
            {
                foreach (Node n in Nodes)
                {
                    if (n.Type == Node.NodeType.Wall)
                    ...
                    n.ShortestPath[n.X, n.Y, (int)initialDir] = new Node.PathInfo(Direction.None, 0);
                    switch (initialDir) {
                        case (Direction.Up):
                            shortestPathAddNode(startNode.Value, startNode.Value.Value.Up, Direction.Up, openList, initialDir);
                            break;
                        case (Direction.Down):
                            shortestPathAddNode(startNode.Value, startNode.Value.Value.Down, Direction.Down, openList, initialDir);
                            break;
                        case (Direction.Left):
                            shortestPathAddNode(startNode.Value, startNode.Value.Value.Left, Direction.Left, openList, initialDir);
                            break;
                        case (Direction.Right):
                            shortestPathAddNode(startNode.Value, startNode.Value.Value.Right, Direction.Right, openList, initialDir);
                            break;
                        case (Direction.None):
                            shortestPathAddNode(startNode.Value, startNode.Value.Value.Up, Direction.Up, openList, initialDir);
                            shortestPathAddNode(startNode.Value, startNode.Value.Value.Down, Direction.Down, openList, initialDir);
                    ...
                    {
                        shortestPathProcessNode(n, openList.First, openList, initialDir);
                    }
                    foreach (Node nClean in Nodes)
                    {
                        nClean.ShortestPath[nClean.X, nClean.Y, (int)initialDir] = null;
                    }
                }
            }
		}

        private void shortestPathProcessNode(Node curNode, LinkedListNode<LinkedListNode<Node>> n, LinkedList<LinkedListNode<Node>> openList, Direction initialDir) {
			openList.Remove(n);
			Node removeNode = n.Value.Value;
            curNode.ShortestPath[removeNode.X, removeNode.Y, (int)initialDir] = removeNode.ShortestPath[removeNode.X, removeNode.Y, (int)initialDir];
            Direction fromDir = removeNode.ShortestPath[removeNode.X, removeNode.Y, (int)initialDir].Direction;
            shortestPathAddNode(n.Value, n.Value.Value.Up, fromDir, openList, initialDir);
            shortestPathAddNode(n.Value, n.Value.Value.Down, fromDir, openList, initialDir);
            shortestPathAddNode(n.Value, n.Value.Value.Left, fromDir, openList, initialDir);
            shortestPathAddNode(n.Value, n.Value.Value.Right, fromDir, openList, initialDir);
		}

        private void shortestPathAddNode(LinkedListNode<Node> n, Node nDir, Direction dir, LinkedList<LinkedListNode<Node>> openList, Direction initialDir) {
            if (nDir.Type == Node.NodeType.Wall || nDir.ShortestPath[nDir.X, nDir.Y, (int)initialDir] != null)
				return;
            nDir.ShortestPath[nDir.X, nDir.Y, (int)initialDir] = new Node.PathInfo(dir, n.Value.ShortestPath[n.Value.X, n.Value.Y, (int)initialDir].Distance + 1, n.Value);
			bool added = false;
			foreach( LinkedListNode<Node> existingNode in openList ) {
                if (existingNode.Value.ShortestPath[existingNode.Value.X, existingNode.Value.Y, (int)initialDir].Distance >
                    nDir.ShortestPath[nDir.X, nDir.Y, (int)initialDir].Distance) {
