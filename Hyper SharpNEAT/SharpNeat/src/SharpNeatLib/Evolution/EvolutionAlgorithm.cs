...
namespace SharpNeatLib.Evolution
{
	public class EvolutionAlgorithm
	{
        ...
        GenomeList[] FastNonDominatedSort(GenomeList P) {
            GenomeList a = new GenomeList();

            List<GenomeList> F = new List<GenomeList>();

            GenomeList[] S = new GenomeList[P.Count];
            int[] n = new int[P.Count];
            for (int p = 0;p<P.Count;p++) {
                ...
                for (int q = 0; q < P.Count; q++) {
                    var q_e = P[q];
                    if (ParetoDominance(p_e, q_e)) {
                        S[p].Add(q_e);
                    } else if (ParetoDominance(q_e, p_e)) {
                        n[p] += 1;
                    }
                }
                if (n[p] == 0) {
                    p_e.MultiObjectiveRank = 1;
                    if (F.Count==0) {
                        F.Add(new GenomeList());
                    }
                    F[0].Add(p_e);
                }
            }
            int i = 0;
            while (F[i].Count != 0) {
                GenomeList Q = new GenomeList();
                for (int p = 0;p<P.Count;p++) {
                    for (int q = 0; q < P.Count; q++) {
                        var q_e = P[q];
                        n[q] = n[p] - 1;
                        if (n[q] == 0) {
                            q_e.MultiObjectiveRank = i + 1;
                            Q.Add(q_e);
                        }
                    }
                }
        ...
        bool ParetoDominance(IGenome v, IGenome u) {
            if (v.MultiObjectiveFitness.Length != u.MultiObjectiveFitness.Length) Console.WriteLine("Hov");
            // For all
            for (int i = 0; i<v.MultiObjectiveFitness.Length;i++) {
                if (v.MultiObjectiveFitness[i]>=u.MultiObjectiveFitness[i]) {
                    // continue
                } else {
                    return false;
                }
        ...
        void CrowdingDistanceAssignment(GenomeList I) {
            int l = I.Count;
            if (l == 0) return;
            foreach (var i in I) {
                i.NSGAiiCrowdDist = 0;
            }
            int mLength = I[0].MultiObjectiveFitness.Length;
            for (int m = 0; m < mLength; m++) {
                I.Sort(new Comparison<IGenome>(delegate(IGenome g1, IGenome g2)
                {
                    if (g1.MultiObjectiveFitness[m] == g2.MultiObjectiveFitness[m]) return 0;
                    if (g1.MultiObjectiveFitness[m] > g2.MultiObjectiveFitness[m]) return 1;
                    return -1;
                }));
                var min = I[0].MultiObjectiveFitness[m];
                var max = I[l-1].MultiObjectiveFitness[m];

                I[0].NSGAiiCrowdDist   = Double.MaxValue;
                I[l-1].NSGAiiCrowdDist = Double.MaxValue;
                if (l>1) {
                    for (int i = 1; i < l - 2; i++) {
                        I[i].NSGAiiCrowdDist = I[i].NSGAiiCrowdDist + (I[i + 1].MultiObjectiveFitness[m] - I[i - 1].MultiObjectiveFitness[m]) / (max - min);
                    }
                }
            }
        }

        public void PerformOneGenerationNSGAii()
        {
            int N = pop.GenomeList.Count;
            //----- Save the parent list
            GenomeList P_t = new GenomeList();
            P_t.AddRange(pop.GenomeList.ToArray());
            ...
            GenomeList Q_t = new GenomeList();
            Q_t.AddRange(pop.GenomeList.ToArray());

            // combine parent and offspring
            GenomeList R_t = P_t;
            R_t.AddRange(Q_t); // combine parent and offspring

            GenomeList[] F = FastNonDominatedSort(R_t);

            GenomeList P_t1 = new GenomeList();
            int i = 0;
            while (P_t1.Count+F[i].Count <= N && F[i].Count != 0) {
                CrowdingDistanceAssignment(F[i]);
                P_t1.AddRange(F[i]);
                i += 1;
            }
            F[i].Sort(new Comparison<IGenome>(delegate(IGenome g1, IGenome g2)
            {
                if ((g1.MultiObjectiveRank > g2.MultiObjectiveRank) ||
                    ((g1.MultiObjectiveRank == g2.MultiObjectiveRank) &&
                    (g1.NSGAiiCrowdDist>g2.NSGAiiCrowdDist))) return 1;
                if ((g1.MultiObjectiveRank == g2.MultiObjectiveRank) &&
                    (g1.NSGAiiCrowdDist == g2.NSGAiiCrowdDist)) return 0;
                return -1;
            }));
            P_t1.AddRange(F[i].GetRange(0, Math.Min(N - P_t1.Count,F[i].Count)));
            //pop.GenomeList.Clear();
            pop.GenomeList.AddRange(P_t1.ToArray());
