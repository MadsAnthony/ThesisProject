...
namespace SharpNeatLib.NeuralNetwork
{
    public class ModularNetwork : INetwork
    {
                                ...
                                ModulePacket[] modules)
        {
            ...
            factorNeuronList = new float[totalNeuronCount];
            for (int i = 0; i < totalNeuronCount; i++) {
                factorNeuronList[i] = 1;
            }
        ...
        private bool SingleStepInternal(double maxAllowedSignalDelta)
        {
            ...
            for (int i = totalInputNeuronCount; i < neuronSignalsBeingProcessed.Length; i++) {
                neuronSignalsBeingProcessed[i] = activationFunctions[i].Calculate(neuronSignalsBeingProcessed[i]+biasList[i])*factorNeuronList[i];
                if (modulatory)
                {
        ...
        public void OverrideHiddenFactorNeuron(float[] inputFactorNeuronList) {
            int j = 0;
            for (int i=totalInputNeuronCount+outputNeuronCount;i<TotalNeuronCount;i++) {
                factorNeuronList[i] = inputFactorNeuronList[j];
                j++;
            }
        }
